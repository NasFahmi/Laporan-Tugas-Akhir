...
Perlu dibedakan secara konseptual antara \textit{client state} dan \textit{server state} dalam pengembangan aplikasi \textit{frontend} modern. \textit{Client state} mengacu pada state yang sepenuhnya berada di sisi klien dan berkaitan langsung dengan antarmuka pengguna, seperti status tampilan, input formulir, filter, atau kondisi navigasi. State ini bersifat sinkron, berumur pendek, dan hanya relevan dalam konteks sesi interaksi pengguna. Sebaliknya, \textit{server state} merepresentasikan data yang bersumber dari sistem eksternal seperti API atau layanan backend, bersifat asinkron, dapat berubah di luar kendali aplikasi klien, serta memerlukan penanganan khusus terkait proses pengambilan data, status pemuatan, penanganan kesalahan, dan sinkronisasi. Perbedaan karakteristik tersebut menjadikan pengelolaan \textit{server state} tidak efektif apabila disamakan dengan \textit{client state}, sehingga diperlukan pendekatan arsitektural tersendiri berupa \textit{Client Data Layer} untuk mengelola data server secara terpusat, konsisten, dan terstruktur.

Dalam konteks pengembangan aplikasi \textit{frontend} yang bersifat \textit{data-driven}, istilah data \textit{frontend} terstruktur merujuk pada cara data yang bersumber dari server dikelola, didistribusikan, dan digunakan di sisi klien secara sistematis. Data yang bersumber dari server dan dikelola melalui mekanisme terpusat di sisi \textit{frontend}, memiliki bentuk dan siklus hidup yang konsisten, serta dapat digunakan ulang oleh berbagai komponen antarmuka tanpa menimbulkan inkonsistensi informasi maupun permintaan data yang tidak terkontrol.

Definisi tersebut menekankan bahwa keterstrukturan data \textit{frontend} ditentukan oleh arsitektur dan mekanisme pengelolaannya. Dengan demikian, struktur data \textit{frontend} berkaitan erat dengan bagaimana data diambil, disimpan sementara, disinkronkan, dan disajikan pada komponen antarmuka.
...
\begin{enumerate}
  ...
  Data yang bersumber dari server dikelola melalui satu lapisan pengelolaan data di sisi \textit{frontend}, sehingga komponen antarmuka tidak melakukan pengambilan data secara langsung ke API. Pendekatan ini menghindari duplikasi data dan memastikan seluruh komponen menggunakan sumber data yang sama.
  ...
  Data \textit{frontend} memiliki status yang terdefinisi dengan jelas, seperti \textit{loading}, \textit{success}, dan \textit{error}. Kejelasan siklus hidup ini memungkinkan sistem untuk memberikan umpan balik yang tepat kepada pengguna serta memudahkan pengelolaan data yang bersifat asinkron.
  \item Pemanfaatan Mekanisme \textit{Caching}
  Data hasil pengambilan dari server disimpan sementara di sisi frontend dan dapat digunakan kembali oleh komponen lain tanpa harus melakukan permintaan ulang ke API. Mekanisme \textit{caching} berperan penting dalam mengurangi permintaan data yang berulang serta meningkatkan efisiensi pengelolaan data.
  \item Sinkronisasi Data Antar-Komponen
  Perubahan data yang terjadi akibat pembaruan atau refetch dapat tercermin secara otomatis pada seluruh komponen yang menggunakan data tersebut. Dengan adanya sinkronisasi, informasi yang ditampilkan pada dashboard tetap konsisten meskipun terjadi interaksi pengguna atau navigasi antar-halaman.
\end{enumerate}
